...
void minirunAnalysis(TString snail_fname, int acc){
  ...
  for(int run = 0; run < all_runs.size(); run++){
    for(int minirun = 0; minirun < all_runs[run].size(); minirun++){
      ...
      TString off_cuts("(laserState==2 || laserState==3) && beamState==1");
      TString cyc_cuts = Form("(firstMPSnumber>=%i && firstMPSnumber<=%i)", all_runs[run][minirun][2], all_runs[run][minirun][3]);
      ...
      for(int m = 0; m < 2; m++){
        for(int las = 0; las < 2; las++){
          quartetwise->Project(names[2*m + las].Data(), meas[m].Data(), Form("%s && %s", lasCuts[las].Data(), cyc_cuts.Data()));
        }
      }
      quartetwise->Draw("epics_ihwp_in>>h_ihwp", cyc_cuts.Data(), "goff");
      ...
      Double_t vOnAsym = vOnDiff/vSubSum;    Double_t vOnAsymE = vOnAsym*TMath::Sqrt(TMath::Power(vSubSumE/vSubSum, 2) + TMath::Power(vOnDiffE/vOnDiff, 2));
      Double_t vOffAsym = vOffDiff/vSubSum; Double_t vOffAsymE = vOffAsym*TMath::Sqrt(TMath::Power(vSubSumE/vSubSum, 2) + TMath::Power(vOffDiffE/vOffDiff, 2));
      Double_t vSubAsym = vOnDiff/vSubSum; Double_t vSubAsymE = vSubAsym*TMath::Sqrt(TMath::Power(vSubSumE/vSubSum, 2) + TMath::Power(vOnDiffE/vOnDiff, 2));
  ...
  if(same_ihwp_state){
    ...
    cout<<"State for snail is "<<state.Data()<<endl;
  }
  else{
    cout<<"Could not see a common ihwp state!"<<endl;
  }
  pPol1->cd();
...
void cycleAnalysis(TString snail_fname, int acc){
  ...
  for(int run = 0; run < all_runs.size(); run++){
    for(int minirun = 0; minirun < all_runs[run].size(); minirun++){
      ...
      TString on_cuts("(laserState==0 || laserState==1) && beamState==1");
      TString off_cuts("(laserState==2 || laserState==3) && beamState==1");
      TString cyc_cuts = Form("((firstMPSnumber>=%i && firstMPSnumber<=%i) || (firstMPSnumber>=%i && firstMPSnumber<=%i) || (firstMPSnumber>=%i && firstMPSnumber<=%i))", 
                              all_runs[run][minirun][2], all_runs[run][minirun][3], all_runs[run][minirun][4], all_runs[run][minirun][5], 
                              all_runs[run][minirun][6], all_runs[run][minirun][7]);
      TString meas[2] = {diff, summ};
      TString lasCuts[2] = {on_cuts, off_cuts};
      TString names[4] = {hON_NameDiff, hOFF_NameDiff, hON_NameSumm, hOFF_NameSumm};
      for(int m = 0; m < 2; m++){
      ...
      TString ihwpState("UNK");
      if(factor == 1){ihwpState = "IN";}
      else if(factor == -1){ihwpState = "OUT";}
      ...
      Double_t vOffSum = hOFF_Summ->GetMean(); Double_t vOffSumE = hOFF_Summ->GetMeanError();
      Double_t vSubSum = vOnSum - vOffSum; Double_t vSubSumE = TMath::Sqrt(TMath::Power(vOnSumE, 2) + TMath::Power(vOffSumE, 2));
      Double_t vOnAsym = vOnDiff/vSubSum; Double_t vOnAsymE = vOnAsym*TMath::Sqrt(TMath::Power(vOnDiffE/vOnDiff, 2) + TMath::Power(vSubSumE/vSubSum, 2));
      Double_t vOffAsym = vOffDiff/vSubSum; Double_t vOffAsymE = vOffAsym*TMath::Sqrt(TMath::Power(vOffDiffE/vOffDiff, 2) + TMath::Power(vSubSumE/vSubSum, 2));
      //Double_t vSubAsym = vOnAsym - vOffAsym; Double_t vSubAsymE = TMath::Sqrt(TMath::Power(vOnAsymE, 2) + TMath::Power(vOffAsymE, 2));
      //Double_t pol = vSubAsym/get_analyzing_power(all_runs[run][minirun][0]); Double_t polE = vSubAsymE/get_analyzing_power(all_runs[run][minirun][0]);
      Double_t pol = vOnAsym/get_analyzing_power(all_runs[run][minirun][0]); Double_t polE = vOnAsymE/get_analyzing_power(all_runs[run][minirun][0]);
      //printf("Run: %i, Laser Cycle: %i; Start Evt: %i; End Evt: %i; IHWP %s; Asym: %.4f ppt\n", 
      //        all_runs[run][minirun][0], all_runs[run][minirun][1], all_runs[run][minirun][2], all_runs[run][minirun][7], ihwpState.Data(), 1000*vSubAsym);
      printf("Run: %i, Laser Cycle: %i; Start Evt: %i; End Evt: %i; IHWP %s; Asym: %.4f ppt\n", 
              all_runs[run][minirun][0], all_runs[run][minirun][1], all_runs[run][minirun][2], all_runs[run][minirun][7], ihwpState.Data(), 1000*vOnAsym);
      //hAsym->SetBinContent(tot_miniruns, 1000*vSubAsym); hAsym->SetBinError(tot_miniruns, 1000*vSubAsymE);
      hAsym->SetBinContent(tot_miniruns, 1000*vOnAsym); hAsym->SetBinError(tot_miniruns, 1000*vOnAsymE);
      hPol->SetBinContent(tot_miniruns, 100*pol); hPol->SetBinError(tot_miniruns, 100*polE);
      if(all_runs[run][minirun][1] % 3 == 1){
        hAsym->GetXaxis()->SetBinLabel(tot_miniruns, Form("%i.%i", all_runs[run][minirun][0], all_runs[run][minirun][1]));
        hPol->GetXaxis()->SetBinLabel(tot_miniruns, Form("%i.%i", all_runs[run][minirun][0], all_runs[run][minirun][1]));
      }
      tot_miniruns++;
