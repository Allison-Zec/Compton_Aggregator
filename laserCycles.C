...
vector<vector<int>> readEventCuts(int runNum){
  ...
  if(!mapfile.good()){
    //printf("No mapfile found for run %i\n", runNum); 
    return eventCuts;
  }
  string readStr;
...
bool isValidCycle(int *cycleInds, vector<int> section1, vector<int> section2, vector<int> section3, int runNum){
  ...
  for(vector<int> cut : eventCuts){
    noEventCuts = noEventCuts && !((cut[0] <= section1[2] && cut[1] >= section1[2]) || 
                                   (cut[0] <= section3[3] && cut[1] >= section3[3]) || 
                                   (cut[0] <= section1[2] && cut[1] >= section3[3]));
  }
  if(!noEventCuts){printf("Cut out cycle ids %i, %i, %i\n", section1[0], section2[0], section3[0]);}
...
vector<vector<int>> refineCycles(vector<vector<int>> cycles){
  ...
  for(int i = 0; i < cycles.size(); i++){
    ...
    if(cycles[i][9] == 1 && cycles[i][5]*1.0/cycles[i][4] > 0.5){
      shortCycles.push_back(cycles[i]);
    }
    //else{
    //  printf("Skipping period from MPS %i to MPS %i because of bad stats...\n", cycles[i][2], cycles[i][3]);
    //}
  }
